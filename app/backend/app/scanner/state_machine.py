...
class PipelineStateMachine:
    ...
    def should_process(state: SymbolState, timeframe: str, candle: Candle | None) -> bool:
        if candle is None or not candle.is_closed:
            return False
        attr = {"15m": "last_processed_15m", "5m": "last_processed_5m", "1m": "last_processed_1m"}[timeframe]
    ...
    @staticmethod
    def evaluate_15m_context(state: SymbolState, candles_15m: tuple[Candle, ...]) -> bool:
        ...
        if not PipelineStateMachine.should_process(state, "15m", latest):
            return bool(state.context_15m.get("context_valid", False))
        ...
        state.context_15m = {
            ...
            "context_valid": valid, "trend": "UP" if up else "DOWN" if down else "NONE",
        ...
        if valid:
            ...
            state.reason_codes = ["HTF_BROKEN"]
        return valid

    @staticmethod
    def evaluate_5m_setup(state: SymbolState, evaluation: StrategyEvaluation) -> bool:
        if state.state == SetupState.COOLDOWN:
            return False
        latest_time = evaluation.latest_entry_candle_time
        if latest_time is not None and state.last_processed_5m == latest_time:
            return bool(state.setup_5m.get("setup_valid", False))
        ...
        if state.state == SetupState.WATCHING and has_signal:
            ...
            state.reason_codes = ["SETUP_VALID"]
            state.trigger_1m["signal"] = evaluation.signal
    ...
    @staticmethod
    def arm_strategy_authority_trigger(state: SymbolState) -> bool:
        """Use the existing approved 5m strategy signal as execution authority.

        This deliberately avoids inventing a new 1m candle-colour trigger. Scanner
        selection only chooses symbols; the existing strategy and Risk Engine remain
        ...
        state.trigger_1m.update({"trigger_status": True, "trigger_reason": "APPROVED_EXISTING_5M_STRATEGY_AUTHORITY"})
